...
def remove_outliers(array,min_remaining_members=2,thresh=0.5):
    ...
    marray = np.ma.masked_array(array,mask=np.zeros_like(array, dtype=bool)) # Create a masked array from the array
    
    outliers_found = True
    num_tot = np.size(marray)
    
    while(outliers_found and (np.size(marray[~marray.mask])>2)):
    ...
    if(np.size(marray[~marray.mask]) < min_remaining_members):
        raise Exception("WARNING: Too few arguments left after removing outliers. (Only " + str(np.size(marray[~marray.mask])) + ")")
    else:
        return marray